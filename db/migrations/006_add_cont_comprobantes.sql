/* ================================================================
   MIGRATION 006 — Agregar tablas cont_maestro_comprobante y
                   cont_detalle_comprobante a todos los tenants
   - Crea las tablas en tenants existentes
   - Recrea sp_create_tenant_schema incluyendo las nuevas tablas
================================================================ */

-- 1. Agregar tablas a tenants existentes
DO $$
DECLARE
    r RECORD;
BEGIN
    ...
    LOOP
        ...
        IF NOT EXISTS (
            ...
            WHERE table_schema = r.schema_name
              AND table_name = 'cont_maestro_comprobante'
        ) THEN
            EXECUTE format('
                CREATE TABLE %I.cont_maestro_comprobante (
                    num_comprobante     SERIAL PRIMARY KEY,
                    fecha_comprobante   TIMESTAMP,
                    tipo                CHAR(10),
                    txt_descripcion     VARCHAR(300),
                    txt_status          INTEGER,
                    txt_id_usuario      VARCHAR(50),
                    fecha_hora_creacion TIMESTAMP DEFAULT NOW(),
                    txt_id_pc           VARCHAR(50),
                    txt_modo_creacion   VARCHAR(50),
                    txt_comentario      VARCHAR(300),
                    clase               VARCHAR(50),
                    num_comp            VARCHAR(50),
                    num_transferencia   INTEGER,
                    sg_moneda           VARCHAR(50),
                    n_tipo              VARCHAR(50),
                    n_sub_tipo          VARCHAR(50),
                    num_operacion_comp  INTEGER,
                    usuario_anulo       VARCHAR(50),
                    fecha_hora_anulo    TIMESTAMP,
                    usuario_modifico    VARCHAR(50),
                    fecha_hora_modifico TIMESTAMP
                )', r.schema_name);
        END IF;
        ...
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.tables
            WHERE table_schema = r.schema_name
              AND table_name = 'cont_detalle_comprobante'
        ) THEN
            EXECUTE format('
                CREATE TABLE %I.cont_detalle_comprobante (
                    id              SERIAL PRIMARY KEY,
                    num_comprobante INTEGER NOT NULL,
                    num_item        INTEGER,
                    txt_cuenta      VARCHAR(50) NOT NULL,
                    nom_corto       VARCHAR(200),
                    txt_tipo        VARCHAR(50),
                    txt_referencia  VARCHAR(50),
                    txt_concepto    VARCHAR(300),
                    num_debito      NUMERIC(25,10) NOT NULL DEFAULT 0,
                    num_credito     NUMERIC(25,10) NOT NULL DEFAULT 0,
                    fecha           TIMESTAMP,
                    auxiliar        INTEGER NOT NULL DEFAULT 0,
                    tipo_auxiliar   CHAR(5)  NOT NULL DEFAULT ''''
                )', r.schema_name);
        END IF;
    END LOOP;
END $$;
...
-- 2. Recrear el procedure con todas las tablas incluidas
...
BEGIN
    ...
    -- Cont Ejercicio
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I.cont_ejercicio (
            id        SERIAL PRIMARY KEY,
            ano       VARCHAR(50),
            fecha_i   TIMESTAMP,
            fecha_f   TIMESTAMP,
    ...
    -- Cont Maestro Comprobante
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I.cont_maestro_comprobante (
            num_comprobante     SERIAL PRIMARY KEY,
            fecha_comprobante   TIMESTAMP,
            tipo                CHAR(10),
            txt_descripcion     VARCHAR(300),
    ...
    -- Cont Detalle Comprobante
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I.cont_detalle_comprobante (
            id              SERIAL PRIMARY KEY,
            num_comprobante INTEGER NOT NULL,
            num_item        INTEGER,
            txt_cuenta      VARCHAR(50) NOT NULL,
...
VALUES('20260408_006', 'Agregar tablas cont_maestro_comprobante y cont_detalle_comprobante', current_user)
