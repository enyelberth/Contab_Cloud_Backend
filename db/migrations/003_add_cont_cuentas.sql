/* ================================================================
   MIGRATION 003 — Agregar tabla cont_cuentas a todos los tenants
   - Crea cont_cuentas en tenants existentes
   - Recrea sp_create_tenant_schema incluyendo cont_cuentas
================================================================ */

-- 1. Agregar cont_cuentas a tenants existentes
DO $$
DECLARE
    r RECORD;
BEGIN
    ...
    LOOP
        IF NOT EXISTS (
            ...
            WHERE table_schema = r.schema_name AND table_name = 'cont_cuentas'
        ) THEN
            EXECUTE format('
                CREATE TABLE %I.cont_cuentas (
                    id               SERIAL,
                ...
                )', r.schema_name, REPLACE(r.schema_name, 'tenant_', ''));
        END IF;
    END LOOP;
END $$;


-- 2. Recrear el procedure incluyendo cont_cuentas
...
VALUES('20260405_003', 'Agregar tabla cont_cuentas (plan de cuentas) a todos los tenants', current_user)
