...
# ==================================================================
# Rutas especiales (deben ir ANTES de las rutas con {company_id})
# ==================================================================
...
def comparar_cuentas(
    origen: int = Query(..., description="ID de la empresa origen"),
    destino: int = Query(..., description="ID de la empresa destino"),
    db=Depends(get_db),
):
    return service.comparar_cuentas(db, origen, destino)
...
def importar_cuentas(
    ...
    return service.importar_cuentas(db, origen, destino, modo)


# ==================================================================
# CRUD estándar por empresa
# ==================================================================
...
def listar_cuentas(company_id: int, db=Depends(get_db)):
...
@router.get(
    "/{company_id}/{txt_cuenta}",
    response_model=schemas.Cuenta,
    summary="Obtener una cuenta por su código",
)
def obtener_cuenta(company_id: int, txt_cuenta: str, db=Depends(get_db)):
...
def crear_cuenta(company_id: int, data: schemas.CuentaCreate, db=Depends(get_db)):
...
def eliminar_cuenta(company_id: int, txt_cuenta: str, db=Depends(get_db)):
