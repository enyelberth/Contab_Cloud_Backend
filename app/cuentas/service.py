...
def _not_found(company_id: int, txt_cuenta: str):
    raise HTTPException(
        status_code=404,
...
def _schema_error(company_id: int):
    raise HTTPException(
        status_code=404,
...
# ------------------------------------------------------------------
def get_cuentas(db, company_id: int):
    ...
    try:
        ...
        db.rollback()
        _schema_error(company_id)


# ------------------------------------------------------------------
# Obtener una cuenta por su código
# ------------------------------------------------------------------
def get_cuenta(db, company_id: int, txt_cuenta: str):
    schema = _schema(company_id)
    try:
        row = fetch_one(
            db,
...
# ------------------------------------------------------------------
def crear_cuenta(db, company_id: int, data: CuentaCreate):
    schema = _schema(company_id)
    try:
        return execute(
            db,
        ...
        if "duplicate key" in str(e).lower():
            raise HTTPException(
                status_code=400,
                detail=f"Ya existe una cuenta con el código '{data.txt_cuenta}' en la empresa {company_id}",
            )
        _schema_error(company_id)
...
# ------------------------------------------------------------------
def actualizar_cuenta(db, company_id: int, txt_cuenta: str, data: CuentaUpdate):
...
# Eliminar una cuenta
# ------------------------------------------------------------------
def eliminar_cuenta(db, company_id: int, txt_cuenta: str):
    get_cuenta(db, company_id, txt_cuenta)
    schema = _schema(company_id)
...
# ------------------------------------------------------------------
def crear_cuentas_lote(db, company_id: int, cuentas: List[CuentaCreate]):
    if not cuentas:
...
# ------------------------------------------------------------------
def comparar_cuentas(db, company_id_origen: int, company_id_destino: int):
...
# ------------------------------------------------------------------
def importar_cuentas(db, company_id_origen: int, company_id_destino: int, modo: str):
    """
    Copia todas las cuentas del schema origen al schema destino.

    modo='omitir'     → ON CONFLICT DO NOTHING  (no toca las existentes)
    modo='reemplazar' → ON CONFLICT DO UPDATE    (sobreescribe las existentes)
    """
    if modo not in ("omitir", "reemplazar"):
        raise HTTPException(status_code=400, detail="modo debe ser 'omitir' o 'reemplazar'")

    schema_origen = _schema(company_id_origen)
    schema_destino = _schema(company_id_destino)
    ...
    try:
        ...
        db.rollback()
        _schema_error(company_id_origen)
